...
const profile = computed(() => profiles.find(p => p.lang === 'es' && p.slug === 'sergio-jurado')!)

const sections = [
  { id: 'perfil', icon: 'i-lucide-user', title: 'Perfil profesional', label: '01' },
  { id: 'experiencia', icon: 'i-lucide-briefcase', title: 'Experiencia', label: '02' },
  { id: 'formacion', icon: 'i-lucide-graduation-cap', title: 'Formación', label: '03' },
  { id: 'skills', icon: 'i-lucide-sparkles', title: 'Skills', label: '04' },
  { id: 'extra', icon: 'i-lucide-info', title: 'Disponibilidad', label: '05' },
]

useSeoMeta({
  title: 'Hola — Sergio Jurado Casado',
  description: 'CV digital de Sergio Jurado Casado: perfil, experiencia, formación y descarga del PDF.',
})
</script>
...
<template>
  <div>
        ...
        <div class="flex flex-col lg:flex-row items-start gap-10 lg:gap-16">
          <div class="flex-1 max-w-2xl">
            ...
            <p class="text-white-400 max-w-2xl leading-relaxed mb-8">
              Esta página es la versión digital de mi CV: el mismo contenido que el PDF,
              más cómodo de leer en el móvil o en el navegador.
            </p>
            ...
            <div class="flex flex-wrap gap-x-5 gap-y-2 text-sm text-white-400 mb-10">
              ...
              <a :href="`tel:${profile.phone.replace(/\s/g, '')}`" class="flex items-center gap-1.5 hover:text-white-100 transition-colors">
                <UIcon name="i-lucide-phone" class="text-teal-400" />
                {{ profile.phone }}
              </a>
              <a :href="`mailto:${profile.email}`" class="flex items-center gap-1.5 hover:text-white-100 transition-colors">
                <UIcon name="i-lucide-mail" class="text-teal-400" />
                {{ profile.email }}
              </a>
              <a :href="profile.linkedin" target="_blank" rel="noopener" class="flex items-center gap-1.5 hover:text-white-100 transition-colors">
                <UIcon name="i-simple-icons-linkedin" class="text-teal-400" />
                LinkedIn
              </a>
            </div>

            <p v-if="profile.note" class="text-sm text-white-500 mb-8">
              {{ profile.note }}
            </p>
    ...
    <section class="py-12 bg-dark-950 border-y border-dark-800/60">
      <UContainer>
        <div class="flex flex-wrap gap-3">
          <a
            ...
            :key="section.id"
            :href="`#${section.id}`"
            class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-dark-700/50 bg-dark-900/50 text-sm text-white-300 hover:border-teal-700/50 hover:text-sky-300 transition-colors"
          >
    ...
    <section id="perfil" class="py-20 bg-dark-900 scroll-mt-24">
    ...
    <section id="experiencia" class="py-20 bg-dark-950 scroll-mt-24">
      <UContainer class="max-w-3xl">
    ...
    <section id="formacion" class="py-20 bg-dark-900 scroll-mt-24">
      <UContainer class="max-w-3xl">
    ...
    <section id="skills" class="py-20 bg-dark-950 scroll-mt-24">
      <UContainer class="max-w-3xl">
        <h2 class="text-3xl font-bold text-white-50 mb-10 flex items-center gap-3">
        ...
        <div class="grid md:grid-cols-2 gap-10 mb-12">
          <div>
            <h3 class="text-lg font-semibold text-white-100 mb-4">Soft skills</h3>
            <ul class="space-y-3">
              <li
          ...
          <div>
            <h3 class="text-lg font-semibold text-white-100 mb-4">Hard skills</h3>
            <ul class="space-y-3">
              <li
    ...
    <section id="extra" class="py-20 bg-dark-900 scroll-mt-24">
      <UContainer class="max-w-3xl">
        <h2 class="text-3xl font-bold text-white-50 mb-6 flex items-center gap-3">
