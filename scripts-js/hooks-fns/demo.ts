...
import { DEEP_MERGE, FROM_JSON, GROUP_BY, HAS_PROPERTIES, IS_OBJECT, TO_JSON } from '@/objects';
import { BYTES_TO_SIZE, CELSIUS_TO_FAHRENHEIT, CURRENCY_FORMAT, KG_TO_LBS, KM_TO_MILES } from '@/classes/convert';
import { STRINGIFY_QUERY } from '@/http';
import { LAMBDA } from '@/classes/lambda';
import { STORAGE_SET, STORAGE_GET, STORAGE_REMOVE } from '@/storage';
...
const setUserPrefs = STORAGE_SET('user_prefs', { theme: 'dark', notifications: true });

// 2. GET (Lo recuperamos ya parseado como objeto)
const getUserPrefs = STORAGE_GET<{ theme: string, notifications: boolean }>('user_prefs');
...
LOGGER('groupBy:', GROUP_BY(
  [{ name: 'Alice', role: 'admin' }, { name: 'Bob', role: 'user' }],
  'role'
));
LOGGER('hasProperties:', HAS_PROPERTIES({ id: 1, name: 'Alice' }, 'id', 'name'));
LOGGER('toJson/fromJson:', FROM_JSON<{ name: string }>(TO_JSON({ name: 'Alice' })));
...
LOGGER('stringifyQuery:', STRINGIFY_QUERY({ search: 'clean code', page: 1, active: true }));
