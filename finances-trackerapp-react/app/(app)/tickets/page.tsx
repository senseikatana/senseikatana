...
export default function TicketsPage() {
  ...
  const [f, setF] = useState<any>({date:'',concept:'',category:'Alimentacion',amount:'',type:'personal',status:'Pendiente',files:[],notes:''});
  ...
  const sc:Record<string,{bg:string,co:string}>={Pendiente:{bg:'var(--color-orange-light)',co:'var(--color-orange)'},Procesado:{bg:'var(--color-blue-light)',co:'var(--color-blue)'},Validado:{bg:'var(--color-green-light)',co:'var(--color-green)'},Archivado:{bg:'var(--color-gray-light)',co:'var(--color-gray)'},Rechazado:{bg:'var(--color-red-light)',co:'var(--color-red)'}};
  ...
  const openE = (i:any) => {setE(i);setF({...i,amount:i.amount.toString()});setFiles([]);setM(true)};
  const handleFile = (ev:React.ChangeEvent<HTMLInputElement>) => {if(ev.target.files)setFiles(p=>[...p,...Array.from(ev.target.files!)])};
  const save = (ev:React.FormEvent) => {ev.preventDefault();const names=files.map(x=>x.name);const p={...f,amount:Number(f.amount),files:[...f.files,...names]};if(e){setData((d:any)=>({...d,tickets:items.map((i:any)=>i.id===e.id?{...i,...p}:i)}))}else{const id=Math.max(0,...items.map((i:any)=>i.id))+1;setData((d:any)=>({...d,tickets:[...items,{id,...p}]}))}setM(false)};
  const del = (i:any) => {if(confirm('Eliminar?'))setData((d:any)=>({...d,tickets:items.filter((x:any)=>x.id!==i.id)}))};
  const delB = (b:any[]) => {const ids=new Set(b.map((i:any)=>i.id));setData((d:any)=>({...d,tickets:items.filter((i:any)=>!ids.has(i.id))}))};
  const total=items.reduce((s:number,i:any)=>s+Number(i.amount),0);

  return (<div>
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
      <div className="p-3 rounded-lg" style={{background:'var(--color-orange-light)'}}><div className="text-xs text-muted-foreground">Pendientes</div><div className="text-xl font-bold" style={{color:'var(--color-orange)'}}>{items.filter((i:any)=>i.status==='Pendiente').length}</div></div>
      <div className="p-3 rounded-lg" style={{background:'var(--color-green-light)'}}><div className="text-xs text-muted-foreground">Validados</div><div className="text-xl font-bold" style={{color:'var(--color-green)'}}>{items.filter((i:any)=>i.status==='Validado').length}</div></div>
    ...
    <CrudTable data={items} onEdit={openE} onDelete={del} onDeleteBatch={delB} onAdd={openC} emptyMessage="Sin tickets" title="Tickets y Recibos" titleIcon="🎫" accentColor="var(--color-primary)" addLabel="+ Nuevo Ticket" columns={[
      ...
      {key:'type',label:'Tipo',format:(i:any)=><span className="badge" style={{background:i.type==='negocio'?'var(--color-blue-light)':'var(--color-gray-light)',color:i.type==='negocio'?'var(--color-blue)':'var(--color-gray-dark)'}}>{i.type==='negocio'?'Negocio':'Personal'}</span>},
      ...
      {key:'status',label:'Estado',format:(i:any)=><span className="badge" style={{background:(sc[i.status]||sc.Pendiente).bg,color:(sc[i.status]||sc.Pendiente).co}}>{i.status}</span>},
      {key:'files',label:'Archivos',format:(i:any)=><span>{i.files&&i.files.length>0?'📎 '+i.files.length:'—'}</span>,hideOnMobile:true},
    ]} />
